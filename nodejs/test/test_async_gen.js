...
b.next().then((v) => {
    console.log(`1st=${JSON.stringify(v)}`)
});
console.log('after 1st next');
b.next().then((v) => {
    console.log(`2nd=${JSON.stringify(v)}`)
});
console.log('after 2nd next');
var c = b.next();
console.log('after 3rd next');
var d = b.return(new Promise(r => {
    setTimeout(() => {
...
var e2 = b.next();
e2.then((v) => {
    console.log(`e2=${JSON.stringify(v)}`)
})
    .then(() => {
        console.log('e2')
    });
