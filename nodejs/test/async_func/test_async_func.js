...
const a = __async(function* (n, v, resolve, awaitRet) {
    console.log(`call=${n}`);
    v = _await(yield Promise.resolve(v));
    console.log(`value=${v}`);
    setTimeout(() => {
        console.log(`timeout=${v}`);
    }, 0);
    v = _await(yield Promise.resolve(v + 1));
    console.log(`value=${v}`);
    if (awaitRet)
        return _await(yield Promise[resolve ? 'resolve' : 'reject'](n + 0.1));
    else
